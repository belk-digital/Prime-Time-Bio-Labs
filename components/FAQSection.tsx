...
export default function FAQSection() {
  ...
  return (
    <section ref={sectionRef} className="py-24 px-6 md:px-12 lg:px-24 bg-white text-gray-900 overflow-hidden border-t border-gray-100">
      <div className="max-w-4xl mx-auto">
        <div className="text-center mb-16 faq-heading">
          <h2 className="text-3xl md:text-5xl font-michroma font-bold uppercase tracking-wider mb-6">
            Frequently Asked <span className="text-transparent bg-clip-text bg-gradient-to-r from-gray-500 to-gray-900">Questions</span>
          </h2>
        ...
        <div className="space-y-4">
          {faqs.map((faq, index) => (
            <div 
              key={index} 
              className={`faq-item border ${openIndex === index ? 'border-indigo-200 bg-indigo-50/30' : 'border-gray-200 bg-white'} rounded-2xl overflow-hidden transition-all duration-300`}
            >
              <button 
              ...
              >
                ...
                <ChevronDown 
                  className={`w-5 h-5 text-gray-500 transition-transform duration-300 ${openIndex === index ? 'rotate-180 text-indigo-600' : ''}`} 
                />
              </button>
